...
uncached, and `Signing.lean` charges a signing failure to six searches: the randomizer search
(`Digest.lean`) and five counter searches (`Counter.lean`, with the code's size from `Code.lean`
...
theorem digestFactor_pow_le : digestFactor ^ digestAttemptLimit ≤ (2⁻¹ : ℝ≥0∞) ^ (2 ^ 11) := by
  have hroom : digestFactor + ((2 ^ 9 : Nat) : ℝ≥0∞)⁻¹ ≤ 1 :=
    digest_room digestReject digestReject_add
  have hhalf := pow_le_half_ennreal (2 ^ 9) (by positivity) digestFactor hroom
  rw [show digestAttemptLimit = 2 ^ 9 * 2 ^ 11 by rw [digestAttemptLimit]; norm_num, pow_mul]
