...
A trial keeps the low `160` bits of a uniform answer, and that truncation is uniform on digests
(`probEvent_uniform_truncateHash_mem`). So one trial accepts exactly as often as a uniform digest
decodes, which by `Code.lean` is at least `2 ^ 150` times in `2 ^ 160`.
-/
...
theorem failMass_encoding_add_le :
  ...
  have hnat : (2 : Nat) ^ 150 ≤ accepted := haccepted ▸ two_pow_le_card_accepting
  have hcard : (Fintype.card Digest : ℝ≥0∞) = (2 : ℝ≥0∞) ^ 160 := by
    rw [show Fintype.card Digest = 2 ^ 160 by simp [digestBits], Nat.cast_pow, Nat.cast_ofNat]
  ...
  have hshare : ((2 : ℝ≥0∞) ^ 10)⁻¹ ≤ (accepted : ℝ≥0∞) / (2 : ℝ≥0∞) ^ 160 := by
    rw [ENNReal.le_div_iff_mul_le (Or.inl (by simp)) (Or.inl (by simp)),
      show (2 : ℝ≥0∞) ^ 160 = 2 ^ 10 * 2 ^ 150 by rw [← pow_add],
