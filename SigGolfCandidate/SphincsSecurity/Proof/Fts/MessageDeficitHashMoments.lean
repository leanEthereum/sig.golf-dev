...
theorem messageDeficit_secondMoment_le (score : ℝ) :
    (1 / 256 : ℝ) * max (score - 255) 0 ^ 2 +
      (255 / 256 : ℝ) * max (score + 1) 0 ^ 2 ≤ max score 0 ^ 2 + 255 := by
  calc
    _ ≤ (1 / 256 : ℝ) * (max score 0 - 255) ^ 2 + (255 / 256 : ℝ) * (max score 0 + 1) ^ 2 := by
      apply add_le_add
      · exact mul_le_mul_of_nonneg_left (positivePart_shift_even_le score (-255) 2 (by decide)) (by norm_num)
...
theorem messageDeficit_secondMoment_ennreal (score : ℝ) :
    (256 : ENNReal)⁻¹ * positiveScoreMoment (score - 255) 2 +
      (1 - (256 : ENNReal)⁻¹) * positiveScoreMoment (score + 1) 2 ≤ positiveScoreMoment score 2 + 255 := by
  have hsub : (1 - (256 : ENNReal)⁻¹).toReal = (255 / 256 : ℝ) := by
...
theorem probEvent_uniformHashOutput_admissible :
    Pr[fun output => Admissible (truncateMessageDigest output) | ($ᵗ HashOutput : ProbComp HashOutput)] = (256 : ENNReal)⁻¹ := by
...
private theorem expected_admissible_choice (accepted rejected : ENNReal) :
    (∑' output, Pr[= output | ($ᵗ HashOutput : ProbComp HashOutput)] *
      ...
      (256 : ENNReal)⁻¹ * accepted + (1 - (256 : ENNReal)⁻¹) * rejected := by
  have hnot : Pr[fun output => ¬Admissible (truncateMessageDigest output) | ($ᵗ HashOutput : ProbComp HashOutput)] = 1 - (256 : ENNReal)⁻¹ := by
...
private theorem expected_messageDeficitMoment_at (parameter : PublicParameter) (root : Digest) (message : Message)
    ...
    (hshift : (256 : ENNReal)⁻¹ * positiveScoreMoment (messageDeficitScore parameter root message cache - 255) power +
      (1 - (256 : ENNReal)⁻¹) * positiveScoreMoment (messageDeficitScore parameter root message cache + 1) power ≤
  ...
  have hafter (output : HashOutput) : messageDeficitMoment parameter root (cache.cacheQuery input output) power =
      (if Admissible (truncateMessageDigest output) then positiveScoreMoment (messageDeficitScore parameter root message cache - 255) power
...
theorem expected_messageDeficitMoment_second_le (parameter : PublicParameter) (root : Digest)
    (cache : QueryCache HashSpec) (hfinite : Finite cache) (input : HashInput) (hfresh : cache input = none) :
    (∑' output, Pr[= output | ($ᵗ HashOutput : ProbComp HashOutput)] *
      messageDeficitMoment parameter root (cache.cacheQuery input output) 2) ≤ messageDeficitMoment parameter root cache 2 + 255 := by
  by_cases hat : ∃ message, MessageInputAt parameter root message input
  · obtain ⟨message, hat⟩ := hat
    exact expected_messageDeficitMoment_at parameter root message cache hfinite input hfresh hat 2 255
