...
theorem stirlingPowerMoment_nineteen_le :
    stirlingPowerMoment (19 / 50) 19 ≤ (2 ^ 33 : ENNReal) := by
...
theorem uniformWordAverage_nearPrice (required : Finset FtsTree) (hdegree : required.card + 1 = Fintype.card FtsTree) :
  ...
  replace hdegree : required.card = 19 := by
  ...
  rw [hdegree]
  refine (mul_le_mul' le_rfl (mul_le_mul' le_rfl stirlingPowerMoment_nineteen_le)).trans ?_
