...
`reference.py` exercises keygen, cache-assisted sign, expand-by-copy, and verify with SHA-256 as a deterministic test oracle. `build_verify.py` assembles the current RV64IM keygen, expand, and verify images in `SigGolfCandidate/SphincsImages.lean`. `check_verify.py` compares the last two to the reference. The separate, expensive `check_keygen.py` verifies the complete keygen image and exact 128 KiB cache; it measured 997,377 compressions and 91,795,978 cycles.
