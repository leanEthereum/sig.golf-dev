...
`reference.py` exercises keygen, cache-assisted sign, expand-by-copy, and verify with SHA-256 as a deterministic test oracle. `build_images.py` assembles all four RV64IM images in `SigGolfCandidate/SphincsImages.lean`; `SigGolfCandidate/SphincsSubmission.lean` fixes their common layout and proposed score. `check_verify.py` compares expand and verify to the reference. `check_keygen.py` verifies the complete keygen image and exact 128 KiB cache; it measured 997,377 compressions and 91,795,978 cycles. `check_sign.py` matches the entire 11,324-byte signature and checks rejection of selected cache corruption; its run used 104,341 compressions.
