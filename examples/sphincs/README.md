...
`SigGolfCandidate/SphincsBridge.lean` proves the abstract byte-string oracle inputs pack injectively into the organizer's length-tagged bit-vector queries, preserving commitment domain separation. `SigGolfCandidate/SphincsVerifierCommitment.lean` identifies the first HASH instruction in the exact verifier image at `0x1130` and proves that a state satisfying the prepared-input invariant sends the correct 60-byte commitment query for eight cycles. Establishing that invariant from the verifier's entry state is still open.
